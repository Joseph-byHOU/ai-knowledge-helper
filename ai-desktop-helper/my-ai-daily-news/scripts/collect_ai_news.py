...
from verge_collector import fetch_verge_ai_news, fallback_verge_ai_news
from github_trending_collector import fetch_github_trending
...
# Hugging Face trending models 收集（直接调用 search_huggingface）
def fetch_huggingface_models(max_results=5):
    """获取 Hugging Face 热门模型"""
    try:
        logger.info("正在获取 Hugging Face 热门模型...")
        models = search_huggingface.search_huggingface(max_results=max_results)
        # 统一字段格式，便于前端展示
        for m in models:
            m.setdefault('type', '模型')
            m.setdefault('tag', '[模型·HuggingFace]')
            m.setdefault('category', m.get('pipeline_tag', ''))
            m.setdefault('published', datetime.now().strftime('%Y-%m-%d'))
        logger.info(f"Hugging Face 模型获取完成: {len(models)}条")
        return models
    except Exception as e:
        logger.error(f"获取 Hugging Face 模型失败: {e}")
        return []


# Reddit AI 热门帖子收集
def fetch_reddit_ai(max_results=5):
    """获取 Reddit r/artificial 热门帖子

    Reddit 的 .json 接口反爬严格(常返回 403),改用更稳定的 RSS feed,
    并使用多域名 + 重试容错,失败时静默跳过该数据源。
    """
    import time

    logger.info("正在获取 Reddit r/artificial 热门帖子...")
    headers = {
    ...
    # Reddit 反爬有速率限制(403/429),多域名 + 重试提高成功率
    candidate_urls = [
    ...
    for url in candidate_urls:
        try:
            ...
            if response.status_code in (403, 429):
                logger.warning(f"Reddit 访问受限({response.status_code}): {url},尝试备选源...")
                time.sleep(2)
                continue
            response.raise_for_status()
            ...
            logger.warning(f"Reddit 源请求失败 {url}: {e}")
            time.sleep(2)

    if not feed or not feed.entries:
        logger.error("获取 Reddit AI 失败: 所有数据源均不可达(可能被反爬限流),已跳过")
        return []
    ...
    try:
        ...
        for entry in feed.entries[:max_results]:
            ...
            # 从 content/summary 中提取纯文本摘要
            raw = ''
            if entry.get('content'):
                raw = entry['content'][0].get('value', '')
            elif entry.get('summary'):
                raw = entry['summary']
            summary = BeautifulSoup(raw, 'html.parser').get_text(strip=True)[:200] if raw else '[外部链接]'

            # 发布日期
            published_parsed = entry.get('published_parsed') or entry.get('updated_parsed')
            if published_parsed:
                published = datetime(*published_parsed[:6]).strftime('%Y-%m-%d')
            else:
        ...
        logger.info(f"Reddit AI 帖子获取完成: {len(posts)}条")
        return posts
    except Exception as e:
        logger.error(f"获取 Reddit AI 失败: {e}")
        return []


# Hacker News AI 热门讨论收集
def fetch_hackernews_ai(max_results=5):
    """获取 Hacker News 上的 AI 相关热门讨论

    使用 Algolia 提供的官方搜索 API,完全开放、无反爬,
    作为社区讨论类数据源(与 Reddit 互补/兜底)。
    """
    try:
        logger.info("正在获取 Hacker News AI 热门讨论...")
        ...
        for hit in data.get('hits', [])[:max_results]:
            ...
            # 优先指向原文,无原文则指向 HN 讨论页
            link = hit.get('url') or f"https://news.ycombinator.com/item?id={object_id}"
            points = hit.get('points', 0)
            num_comments = hit.get('num_comments', 0)
            created = hit.get('created_at', '')[:10] or datetime.now().strftime('%Y-%m-%d')
            posts.append({
                'type': '资讯',
                'tag': '[资讯·Hacker News]',
                'title': title,
                'summary': f"{points} points · {num_comments} 评论 · HN 社区热议",
                'url': link,
                'published': created,
                'source': 'hackernews',
                'category': 'hackernews'
        ...
        logger.info(f"Hacker News AI 讨论获取完成: {len(posts)}条")
        return posts
    except Exception as e:
        logger.error(f"获取 Hacker News AI 失败: {e}")
        return []
...
def main():
    ...
    # 6. 收集The Verge AI新闻（带浏览器备选）
    if config.get('sources', {}).get('theverge', {}).get('enabled', True):
        tv_config = config['sources']['theverge']
        tv_content = try_with_fallback(
            fetch_verge_ai_news,
            fallback_verge_ai_news,
            max_items=tv_config.get('max_items', 5),
            days_back=tv_config.get('days_back', 1)
        )
        all_news.extend(tv_content)
    
    # 7. 收集其他自定义RSS源（带浏览器备选）
    ...
    if github_config.get('enabled', True):
        ...
        github_projects = fetch_github_trending(
            max_results=github_config.get('max_results', 3)
        )
        all_news.extend(github_projects)

    # 9. 收集 Hugging Face 热门模型
    hf_models_config = config.get('sources', {}).get('huggingface_models', {})
    if hf_models_config.get('enabled', True):
        logger.info("正在获取 Hugging Face 热门模型...")
        hf_models = fetch_huggingface_models(
            max_results=hf_models_config.get('max_results', 5)
        )
        all_news.extend(hf_models)

    # 10. 收集 Reddit AI 热门帖子
    reddit_config = config.get('sources', {}).get('reddit', {})
    if reddit_config.get('enabled', True):
        logger.info("正在获取 Reddit AI 热门帖子...")
        reddit_posts = fetch_reddit_ai(
            max_results=reddit_config.get('max_results', 5)
        )
        all_news.extend(reddit_posts)

    # 11. 收集 Hacker News AI 热门讨论(社区讨论源,与 Reddit 互补/兜底)
    hn_config = config.get('sources', {}).get('hackernews', {})
    if hn_config.get('enabled', True):
        hn_posts = fetch_hackernews_ai(
            max_results=hn_config.get('max_results', 5)
        )
        all_news.extend(hn_posts)

    # 限制总数
    max_total = config.get('output', {}).get('max_total_items', 25)
